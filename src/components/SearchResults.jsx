  ...
  return (
    <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-1000 w-[90%] max-w-md bg-white rounded-2xl shadow-lg max-h-64 overflow-y-auto">
      <div className="p-3 border-b border-gray-100">
        <p className="text-xs text-gray-500 text-center">{results.length} result{results.length > 1 ? 's' : ''} — sorted cheapest first</p>
      </div>
      {results.map((item, index) => {
        const stale = isStale(item.recorded_at)
        const isCheapest = item.price === cheapest
        const isMostExpensive = item.price === mostExpensive && results.length > 1
        ...
        return (
          ...
          >
            <div>
              ...
              <p className="text-xs text-gray-400 capitalize">{item.stores?.type}</p>
              {stale && (
                <p className="text-xs text-orange-400">⚠️ Price may be outdated</p>
              )}
            </div>
            <div className="text-right">
              <p className={`font-bold text-lg ${isCheapest ? 'text-green-500' : isMostExpensive ? 'text-red-400' : 'text-gray-700'}`}>
                ₱{parseFloat(item.price).toFixed(2)}
              </p>
              {isCheapest && <p className="text-xs text-green-400">Cheapest</p>}
              {isMostExpensive && <p className="text-xs text-red-400">Most expensive</p>}
            </div>
          </button>
        )
      })}
