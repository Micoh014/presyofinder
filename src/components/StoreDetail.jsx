...
export default function StoreDetail({ store, onClose }) {
  ...
  async function handleAddItem() {
    ...
    if (!itemPrice || isNaN(itemPrice)) return alert('Please enter a valid price.')
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-1000">
      <div className="bg-white w-full max-w-md rounded-t-2xl p-6 space-y-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          {items.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-4">No items yet. Add one above.</p>
          )}
          {items.map(item => (
            <div key={item.id} className="flex justify-between items-center border border-gray-100 rounded-lg px-4 py-3">
